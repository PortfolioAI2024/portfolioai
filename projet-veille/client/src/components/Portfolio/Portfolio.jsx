  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const updatedFieldsList = Object.keys(changedFields)
        .map((field) => {
          ...
          switch (field) {
            ...
            default:
              return field; // Default case if no custom name is needed
          }
        })
        .join(", ");
      ...
      if (updatedFieldsList) {
        alert(
          `The following information in your portfolio was updated successfully: ${updatedFieldsList}.`
        );
      } else {
      ...
      alert(
        "An error occurred while updating your portfolio. Please try again."
      );
    }
  };
  ...
  return (
    <section className="portfolio mx-auto">
      <form onSubmit={handleSubmit} className="mt-4">
        ...
        <div className="items-center">
          ...
          <span className="flex">
            <input
              ...
              onChange={(e) => setNewCompetenceTechnique(e.target.value)}
              className="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
            />
            <button
              type="button"
        ...
        <ul className="inline">
          {formData.competencesTechniques.map((competenceTechniques, index) => (
            <li key={index} className="flex flex-row">
              <button
                type="button"
                onClick={() => handleRemoveCompetenceTechnique(index)}
                className="m-1 bg-red-500 text-white font-semibold py-1 px-2 ml-2 rounded-md hover:bg-red-600 transition-colors duration-300"
              >
                Supprimer
              </button>
